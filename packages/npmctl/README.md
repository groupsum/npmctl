...
The release workflow publishes all npmctl distributions through GitHub Actions
OIDC first, then falls back to the configured PyPI API token only if trusted
publishing is unavailable. Configure a pending trusted publisher on PyPI for
each published project before dispatching a release:
...
The GitHub workflow grants `id-token: write` and publishes from the `pypi`
environment, so the PyPI project settings must match those fields exactly.
